    ...
    function initSliders() {
        ...
        if (document.querySelector(".products-slider__slider")) new core(".products-slider__slider", {
            ...
            on: {
                init: function(swiper) {
                    ...
                    const allSlides = sliderContainer.querySelector(".fraction-products-slider__all");
                    const allSlidesItems = swiper.slides.filter((slide => !slide.classList.contains("swiper-slide-duplicate")));
                    const totalSlides = allSlidesItems.length;
                    allSlides.innerHTML = totalSlides < 10 ? `0${totalSlides}` : totalSlides;
                },
                slideChange: function(swiper) {
                    const sliderContainer = swiper.el;
                    const currentSlide = sliderContainer.querySelector(".fraction-products-slider__current");
        ...
        if (document.querySelector(".new-products__slider")) new core(".new-products__slider", {
            ...
            on: {
                init: function(swiper) {
                    const sliderContainer = swiper.el;
                    const allSlides = sliderContainer.querySelector(".fraction-new-products__all");
                    const allSlidesItems = swiper.slides.filter((slide => !slide.classList.contains("swiper-slide-duplicate")));
                    const totalSlides = allSlidesItems.length;
                    allSlides.innerHTML = totalSlides < 10 ? `0${totalSlides}` : totalSlides;
                },
                slideChange: function(swiper) {
                    const sliderContainer = swiper.el;
                    const currentSlide = sliderContainer.querySelector(".fraction-new-products__current");
                    const currentSlideIndex = swiper.realIndex + 1;
                    currentSlide.innerHTML = currentSlideIndex < 10 ? `0${currentSlideIndex}` : currentSlideIndex;
                }
            }
